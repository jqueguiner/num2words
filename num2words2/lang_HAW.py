...
class Num2Word_HAW(Num2Word_Base):
    ...
    def setup(self):
        ...
        self.ones = ['', "'ekahi", "'elua", "'ekolu", "'ehā", "'elima", "'eono", "'ehiku", "'ewalu", "'eiwa"]
        self.tens = ['', "'umi", 'iwakālua', 'kanakolu', 'kanahā', 'kanalima', 'kanaono', 'kanahiku', 'kanawalu', 'kanaiwa']
        self.hundred = "haneli"
        self.thousand = "kaukani"
        self.million = "miliona"

    def to_cardinal(self, number):
        """Convert a number to its word representation in Hawaiian."""
        n = str(number).strip()
    ...
    def _int_to_word(self, number):
        """Convert an integer to its word representation in Hawaiian."""
        if number == 0:
    ...
    def to_ordinal(self, number):
        """Convert to ordinal in Hawaiian.

        Hawaiian ordinals use 'ka' prefix for most numbers.
        Special forms exist for 1st (ka mua) and 2nd (ka lua).
        """
        if number == 1:
            return "ka mua"  # first
        elif number == 2:
            return "ka lua"  # second
        else:
            # For other numbers, use "ka" + cardinal
