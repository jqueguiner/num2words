...
class Num2Word_LB(Num2Word_Base):
    CURRENCY_FORMS = {
    ...
    def setup(self):
        ...
        self.ones = ['', 'eent', 'zwou', 'dräi', 'véier', 'fënnef', 'sechs', 'siwen', 'aacht', 'néng']
        self.tens = ['', 'zéng', 'zwanzeg', 'drësseg', 'véierzeg', 'fofzeg', 'sechzeg', 'siwwenzeg', 'achtzeg', 'nongzeg']
        self.hundred = "honnert"
        self.thousand = "dausend"
        self.million = "Millioun"

    def to_cardinal(self, number):
        """Convert a number to its word representation in Luxembourgish."""
        n = str(number).strip()
    ...
    def _int_to_word(self, number):
        """Convert an integer to its word representation in Luxembourgish."""
        if number == 0:
    ...
    def to_ordinal(self, number):
        """Convert to ordinal in Luxembourgish."""
