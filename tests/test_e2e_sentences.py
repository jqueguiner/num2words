...
class TestNum2WordsSentencesE2E(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not csv_file.exists():
            print(f"Warning: {csv_file} not found. Run generate_test_e2e_sentences.py first.")
            return
        ...
        try:
            with open(csv_file, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    if row.get('lang') and row.get('input_sentence') and row.get('expected_output'):
        ...
        if failures:
            ...
            for lang in sorted(by_lang.keys()):
                ...
                for fail in by_lang[lang][:3]:  # Show first 3 failures per language
                    print(f"  Test #{fail['index']}: {fail['description']} [{fail['test_type']}]")
        ...
        for lang in priority_langs:
            ...
            with self.subTest(lang=lang):
                ...
                for case in lang_cases[:5]:  # Test first 5 cases per language
                    lang_code = lang.replace('_', '-')
                    try:
                        result = num2words_sentence(case['input_sentence'], lang=lang_code)
                ...
                self.assertLess(failures, len(lang_cases[:5]), 
...
class TestNum2WordsSentenceManual(unittest.TestCase):
    ...
    def test_english_basic(self):
        ...
        test_cases = [
            ...
            ("It costs $99.99", "It costs $ninety-nine point nine nine"),
