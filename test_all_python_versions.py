...
This script helps test the package locally on Python 3.8, 3.9, 3.10, 3.11, 3.12, and 3.13
using pyenv or Docker to manage different Python versions.

Usage:
    python test_all_python_versions.py [--method pyenv|docker] [--versions 3.8,3.9,3.10,3.11,3.12,3.13]
...
def test_python_version_pyenv(version, project_dir):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        shutil.copytree(project_dir, temp_project, ignore=shutil.ignore_patterns('.git', '__pycache__', '*.pyc', '.tox', 'venv', 'env'))
        
        # Set Python version for this directory
        returncode, stdout, stderr = run_command(f"pyenv local {version}", cwd=temp_project)
        if returncode != 0:
        ...
        returncode, stdout, stderr = run_command("python -m venv venv", cwd=temp_project)
        if returncode != 0:
        ...
        returncode, stdout, stderr = run_command(f'"{venv_pip}" install -e .', cwd=temp_project)
        if returncode != 0:
        ...
        if test_requirements.exists():
            ...
            returncode, stdout, stderr = run_command(f'"{venv_pip}" install -r requirements-test.txt', cwd=temp_project)
            if returncode != 0:
                print(f"⚠️  Warning: Failed to install test requirements: {stderr}")
        
        # Run tests
        print("🧪 Running tests...")
        returncode, stdout, stderr = run_command(f'"{venv_python}" -m pytest tests/ -v', cwd=temp_project)
...
def test_python_version_docker(version, project_dir):
...
def main():
    parser = argparse.ArgumentParser(description='Test num2words2 across multiple Python versions')
    parser.add_argument('--method', choices=['pyenv', 'docker'], default='pyenv',
                       help='Method to use for testing (default: pyenv)')
    parser.add_argument('--versions', default='3.8,3.9,3.10,3.11,3.12,3.13',
    ...
    for version in versions:
        if args.method == 'pyenv':
            ...
            success = test_python_version_pyenv(version, project_dir)
        elif args.method == 'docker':
            success = test_python_version_docker(version, project_dir)
    ...
    print(f"\n📈 Results: {passed} passed, {failed} failed out of {len(results)} versions")
    ...
    if failed > 0:
        ...
        for version, success in results.items():
            if not success:
                if args.method == 'pyenv':
                    print(f"   pyenv local {version} && python -m pytest tests/ -v")
                elif args.method == 'docker':
