...
def find_python_files(directory):
    ...
    for root, dirs, files in os.walk(directory):
        ...
        dirs[:] = [d for d in dirs if d not in {'.git', '__pycache__', '.tox', 'venv', 'env', '.env', 'node_modules'}]
...
def migrate_file_content(content):
    ...
    replacements = [
        (r'from\s+num2words\.([a-zA-Z_][a-zA-Z0-9_]*)\s+import', r'from num2words2.\1 import', 'Updated submodule import'),
        (r'from\s+num2words\s+import', 'from num2words2 import', 'Updated import statement'),
        (r'(^|\s)import\s+num2words(?!\w)', r'\1import num2words2 as num2words', 'Updated import with alias'),
    ]
    ...
    for pattern, replacement, description in replacements:
        ...
        if matches:
            ...
            changes_made.append(f"  - {description} ({len(matches)} occurrence{'s' if len(matches) != 1 else ''})")
...
def main():
    ...
    print(f"🔍 Scanning for num2words imports in: {os.path.abspath(args.directory)}")
    print()
    ...
    if args.requirements:
        req_patterns = ['requirements*.txt', 'requirements*.in', 'pyproject.toml', 'setup.py', 'setup.cfg']
    ...
    print(f"📝 Found num2words imports in {len(migrated_files)} file{'s' if len(migrated_files) != 1 else ''}:")
    print()
    ...
    print()
    print("📚 For more information, see: https://github.com/jqueguiner/num2words#migration")
