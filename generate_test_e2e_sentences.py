...
def get_language_names():
    ...
    return {
        ...
        'tr': 'Turkish', 'pl': 'Polish', 'cs': 'Czech', 'sk': 'Slovak',
        'hu': 'Hungarian', 'ro': 'Romanian', 'bg': 'Bulgarian', 'el': 'Greek',
        'sv': 'Swedish', 'no': 'Norwegian', 'da': 'Danish', 'fi': 'Finnish',
        'is': 'Icelandic', 'et': 'Estonian', 'lv': 'Latvian', 'lt': 'Lithuanian',
        'uk': 'Ukrainian', 'be': 'Belarusian', 'sr': 'Serbian', 'hr': 'Croatian',
        'sl': 'Slovenian', 'mk': 'Macedonian', 'sq': 'Albanian', 'ka': 'Georgian',
        'hy': 'Armenian', 'az': 'Azerbaijani', 'kk': 'Kazakh', 'uz': 'Uzbek',
        'tk': 'Turkmen', 'tg': 'Tajik', 'mn': 'Mongolian', 'fa': 'Persian',
        'ur': 'Urdu', 'ps': 'Pashto', 'sd': 'Sindhi', 'pa': 'Punjabi',
        'gu': 'Gujarati', 'mr': 'Marathi', 'kn': 'Kannada', 'ml': 'Malayalam',
        'si': 'Sinhala', 'ne': 'Nepali', 'as': 'Assamese', 'km': 'Khmer',
        'lo': 'Lao', 'my': 'Burmese', 'bo': 'Tibetan', 'am': 'Amharic',
        'ha': 'Hausa', 'yo': 'Yoruba', 'sw': 'Swahili', 'sn': 'Shona',
        'af': 'Afrikaans', 'id': 'Indonesian', 'ms': 'Malay', 'jw': 'Javanese',
        'tl': 'Tagalog', 'haw': 'Hawaiian', 'mi': 'Maori', 'mg': 'Malagasy',
        'mt': 'Maltese', 'cy': 'Welsh', 'ga': 'Irish', 'gl': 'Galician',
        'eu': 'Basque', 'ca': 'Catalan', 'oc': 'Occitan', 'br': 'Breton',
        'fo': 'Faroese', 'lb': 'Luxembourgish', 'yi': 'Yiddish', 'eo': 'Esperanto',
        'la': 'Latin', 'sa': 'Sanskrit', 'tet': 'Tetum', 'ht': 'Haitian Creole',
        'wo': 'Wolof', 'ln': 'Lingala', 'so': 'Somali', 'su': 'Sundanese',
        'ce': 'Chechen', 'ba': 'Bashkir', 'tt': 'Tatar', 'bs': 'Bosnian',
        'nn': 'Norwegian Nynorsk', 'kz': 'Kazakh', 'en-in': 'Indian English',
        'en-ng': 'Nigerian English', 'es-co': 'Colombian Spanish',
        'es-cr': 'Costa Rican Spanish', 'es-gt': 'Guatemalan Spanish',
        'es-ni': 'Nicaraguan Spanish', 'es-ve': 'Venezuelan Spanish',
        'fr-be': 'Belgian French', 'fr-ch': 'Swiss French', 'fr-dz': 'Algerian French',
...
def generate_sentence_test_cases(client: OpenAI, count: int = 100, 
    ...
    batch_size = 10 if len(test_langs) > 10 else 20
    all_test_cases = []
    ...
    for i, lang in enumerate(test_langs):
        ...
        try:
            print(f"  Generating {current_count} test cases for {lang} ({lang_name})...")
...
def write_sentence_tests_to_csv(test_cases: List[Dict], output_file: str, append: bool = False):
    ...
    # Define CSV headers
    headers = ['lang', 'input_sentence', 'expected_output', 'english_translation', 'description', 'test_type']
...
def main():
    parser = argparse.ArgumentParser(description='Generate num2words_sentence test cases using OpenAI')
    parser.add_argument('--output', '-o', type=str, default='test_e2e_sentences.csv',
    ...
    if args.sample:
        ...
        if not args.languages:
            args.languages = ['en', 'fr', 'es', 'de', 'ja']  # Sample of diverse languages
    ...
    test_cases = generate_sentence_test_cases(client, args.count, args.languages, args.model)
